...
def run_web_ui() -> None:
    ...
    create_ui().queue().launch(share=gradio_share, server_name="127.0.0.1", inbrowser=True)
...
def run_web_demo() -> None:
    ...
    gradio_share = os.getenv("GRADIO_SHARE", "0").lower() in ["true", "1"]
    server_name = os.getenv("GRADIO_SERVER_NAME", "[::]" if gradio_ipv6 else "0.0.0.0")
    create_web_demo().queue().launch(share=gradio_share, server_name="127.0.0.1", inbrowser=True)
